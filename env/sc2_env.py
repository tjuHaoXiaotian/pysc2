...
class SC2Env(environment.Base):
  ...
  def __init__(self,  # pylint: disable=invalid-name
    ...
    Args:
      ...
      feature_units: Whether to include feature unit data in observations
    Raises:
  ...
  def _setup(self,
    ...
    interface = sc_pb.InterfaceOptions(
        raw=(visualize or feature_units), score=True,
    ...
    # f = open("static_data.txt", "a") # 打开文件以便写入
    # print("games static data: ")
    # static_data.print_static_datas(f)
    #
    # f.flush()
    # f.close()
  ...
  def step(self, actions):
    ...
    # TODO: RemoteController 中几个非常关键的函数！！！！！！！！！！！！
    # print("controller: ", c)
    # print("c.game_info(): ", c.game_info())
    # print("c.data(): ", c.data())
    # print("c.observe(): ", c.observe())

    o = self._obs[0]
    acs = []
    for i in range(0,len(actions)):
      try:
        ac = (c.act, self._features.transform_action(o.observation, actions[i]))
        # print("转换后的sc2的函数: ", ac)
        acs.append(ac)
  ...
  def my_print(self):
    f = open("observations/controller/game_infos/game_info{}.txt".format(time.time()), "a")  # 打开文件以便写入
    np.set_printoptions(threshold=np.inf)  # 全部输出
    for c in self._controllers:
      # print(o.observation.raw_data.units,file=f)
      print(c.game_info(), file=f)
    ...
    # f = open("observations/controller/datas/data{}.txt".format(time.time()), "a")  # 打开文件以便写入
    # np.set_printoptions(threshold=np.inf)  # 全部输出
    # for c in self._controllers:
    #   # print(o.observation.raw_data.units,file=f)
    #   print(c.data(), file=f)
    ...
    # f.close()
    #
    # f = open("observations/controller/c/c{}.txt".format(time.time()), "a")  # 打开文件以便写入
    # np.set_printoptions(threshold=np.inf)  # 全部输出
    # for c in self._controllers:
    #   # print(o.observation.raw_data.units,file=f)
    #   print(c, file=f)
