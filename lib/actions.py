...
TYPES = Arguments.types(
    screen=ArgumentType.point(), # 0
    minimap=ArgumentType.point(), # 1
    screen2=ArgumentType.point(), # 2
    queued=ArgumentType.enum([False, True]),  # (now vs add to queue) # 3
    control_group_act=ArgumentType.enum([ # 4
    ...
    control_group_id=ArgumentType.scalar(10),  # 5
    select_point_act=ArgumentType.enum([  # 6
    ...
    select_add=ArgumentType.enum([False, True]),  # (select vs select_add)   # 7
    select_unit_act=ArgumentType.enum([  # 8
    ...
    select_unit_id=ArgumentType.scalar(500),  # Depends on current selection.  # 9
    select_worker=ArgumentType.enum([  # 10
    ...
    build_queue_id=ArgumentType.scalar(10),  # Depends on current build queue.  # 11
    unload_id=ArgumentType.scalar(500),  # Depends on the current loaded units. # 12
)
