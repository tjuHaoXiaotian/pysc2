...
# from pysc2.agents.test.common.world_to_feature import world_to_feature
...
class Features(object):
  ...
  def __init__(self, game_info=None, map_size=None,camera_width_world_units=None, screen_size_px=None, minimap_size_px=None,
  ...
  def observation_spec(self):
    ...
    if self._feature_units:
        obs_spec["my_units"] = (0, 7)
        obs_spec["my_opponents"] = (0, 7)
  ...
  # TODO: update the original observation with raw data (obs.raw_data.units)
  @sw.decorate
  def transform_obs(self, obs):
    ...
    # print("屏幕尺寸：",self._screen_size_px)
    def translate_x(x):
        return 3 + (self._screen_size_px.x - 4 - 3) * (x - 22) / 22
    def translate_y(y):
        return 3 + (self._screen_size_px.y - 2 - 3) * (16 / 22) * (1 - (y -20) / (36 - 20))

    if self._feature_units:
        def unit_vec_raw(u):
            # pos = world_to_feature.word_to_screen(u.pos.x, u.pos.y)
        ...
        data_raw = obs.raw_data

        if data_raw and len(data_raw.units) > 0 and self._map_size is not None:
            with sw("raw_data"):
                # Update the camera location so we can calculate world to screen positions
                self._update_camera(point.Point.build(data_raw.player.camera))
                friends = list(unit_vec_raw(u) for u in data_raw.units if u.owner == 1)
                if friends and len(friends) > 0:
                    # 按 tag 从低到高排序
                    friends = sorted(friends, key=lambda x: x[0])
                    out["my_units"] = np.stack(friends)
                opponents = list(unit_vec_raw(u) for u in data_raw.units if u.owner == 2)
                if opponents and len(opponents) > 0:
                    opponents = sorted(opponents, key=lambda x: x[0])
                    out["my_opponents"] = np.stack(opponents)
