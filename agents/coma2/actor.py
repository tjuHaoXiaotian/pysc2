...
class Actor(object):
    ...
    def _build_net(self, state_inputs_friends, state_inputs_enemies, scope, trainable):
        ...
        with tf.variable_scope(scope):
            ...
            state_inputs_friends = tf.reshape(state_inputs_friends, [-1, COMA_CFG.seq_length_friends, COMA_CFG.state_dim])
            state_inputs_enemies = tf.reshape(state_inputs_enemies, [-1, COMA_CFG.seq_length_enemies, COMA_CFG.state_dim])

            state_feature_friends = self._basic_lstm_layer(state_inputs_friends, self.batch_size, self.sequence_length_friends, self.keep_prob)
            state_feature_enemies = self._basic_lstm_layer(state_inputs_enemies, self.batch_size, self.sequence_length_enemies, self.keep_prob)
            layer2 = self._fully_connected(tf.concat([state_feature_friends, state_feature_enemies], axis=1),
                                           [COMA_CFG.lstm_size * 2, 128], [128],
    ...
    #     })
    def operation_cal_softmax_probablility(self, batch_size, state, sequence_len_batch, is_training):
        '''
    ...
    def operation_choose_action(self, batch_size, state, sequence_length, is_training):
        '''
        :param batch_size: RNN batch size
        :param state:
        :param is_training:
    ...
    def operation_greedy_action(self, batch_sequence_len, batch_size, state, is_training):
        '''
        :param batch_size: RNN batch size
        :param state:
        :param is_training:
        ...
        if len(batch_sequence_len) == 1 and batch_sequence_len[0] == 0: # 全死掉了
            return None
        prob_weights = self.sess.run(self.target_softmax_action_outputs,feed_dict={
            self.batch_size: batch_size,
            self.sequence_length: batch_sequence_len,
            self.state_input: state,
            self.is_training: is_training,
            self.keep_prob: 1.,
            # self.initial_state: new_cell_state
    ...
    def operation_actor_learn(self, batch_size, state, sequence_len_batch, execute_action, advantage, is_training):
        '''
