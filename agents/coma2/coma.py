...
from pysc2.agents.coma2.env_until import cal_local_observation_for_unit,convert_discrete_action_2_sc2_action,one_hot_action
from pysc2.agents.coma2.replay_buffer import construct_transition, ReplayBuffer
from pysc2.agents.coma2.actor import Actor
from pysc2.agents.coma2.ciritic import Critic
import os
# ==========================================features =================================================
# feature 玩家 0, 1
...
COMA_CFG.batch_size = 64
COMA_CFG.replay_buff_size = 16000 #10**6  # 1M
COMA_CFG.replay_buff_save_segment_size = 8000 #30*3000  # every 180,000 Transition data.
...
COMA_CFG.lstm_size = 64
COMA_CFG.seq_length_friends = COMA_CFG.agent_num
COMA_CFG.seq_length_enemies = COMA_CFG.enemy_num
...
class Coma(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if not self.is_roach_defeated(obs):
            if self.isLast(obs): # 我输了
                ...
                # 存储 (结局状态不重要)
                self.save_transition(self.pre_alive_order, self.pre_alive_units_count, self.pre_observation, self.pre_actions, reward, self.pre_alive_order, self.pre_alive_units_count, self.pre_observation, True)
                ...
                alive_units_count_to_save = {}
                states_to_save = {}
                actions_to_save = {}
                alive_order_to_save = {}
                select_actions = []
                for friend in alive_friends:
                    alive_units_count, local_observation, alive_friends_order = cal_local_observation_for_unit(friend, alive_friends, alive_enemies, self.friends_tag_2_id)
                    if self.is_training:
                        selected_action_id,_ = self.actor.operation_choose_action(1,local_observation, alive_units_count, is_training=False)
                        select_actions.append(_)
                    else:
                        selected_action_id = self.actor.operation_greedy_action(1,local_observation, alive_units_count, is_training=False)
                    ...
                    # 保存 state 和 action
                    alive_units_count_to_save[self.friends_tag_2_id[friend[0]]] = alive_units_count
                ...
                if not self.isFirst(obs) and self.is_training:
                    # 存储
                    self.save_transition(self.pre_alive_order, self.pre_alive_units_count, self.pre_observation, self.pre_actions, reward, alive_order_to_save, alive_units_count_to_save, states_to_save, False)

                # 6: update prev properties
                self.pre_alive_units_count = alive_units_count_to_save
        ...
        if self.is_training and self.replay_buffer.length >= COMA_CFG.batch_size * 2 and self.win_times <= 2 and self.steps % COMA_CFG.training_every_steps == 0:
            state_batch, sequence_len_batch, action_others_batch, action_batch, reward_batch, next_state_batch, sequence_len_s_batch, next_state_others_batch, sequence_lens_s_batch, terminated_batch = self.replay_buffer.sample_batch(COMA_CFG.batch_size)
            # training critic:
            # 1: 准备 batch 数据
            action_others_batch_s_ = []
            # TODO: 可能会只有一个（当前单位已经死亡）
            for sequence_lens_s_, next_state_others in zip(sequence_lens_s_batch,next_state_others_batch):  # 对于每一个单位：其他所有单位的观察
                # 还没有 one-hot
                action_others_s_ = self.actor.operation_greedy_action(sequence_lens_s_, len(next_state_others), next_state_others, is_training=True)
                action_per = []
            ...
            batch_td_target = self.critic.operation_get_TDtarget(
                len(next_state_batch),
                next_state_batch,
                sequence_len_s_batch,
                action_others_batch_s_,  # 已经对齐了，11 * 8
                reward_batch,
                terminated_batch,
                is_training=True
            )
            self.critic.operation_critic_learn(len(state_batch), state_batch, sequence_len_batch, action_others_batch, action_batch, batch_td_target, is_training=True)

            # training actor
            # 3: calculate advantage
            actor_output_probability = self.actor.operation_cal_softmax_probablility(len(state_batch), state_batch, sequence_len_batch, is_training=True)
            batch_advantages = self.critic.operation_cal_advantage(len(state_batch), state_batch, sequence_len_batch, action_others_batch, action_batch, actor_output_probability, is_training=True)
            # update actor
            cost = self.actor.operation_actor_learn(len(state_batch), state_batch, sequence_len_batch, action_batch, batch_advantages, is_training=True)
            ...
            if self.steps % COMA_CFG.log_every_steps == 0:
                feed_dict = {
                    ...
                    self.actor.sequence_length: sequence_len_batch,
                    ...
                    self.critic.sequence_length: sequence_len_batch,
                    ...
                    self.critic.batch_size: len(state_batch),
    ...
    def save_transition(self, all_alive_order_s, all_alive_units_count, all_alive_s, all_alive_a, all_r, all_alive_order_s_, all_alive_units_count_s_, all_alive_s_, all_done):
        '''
        :param all_alive_order_s: dict, 当前存活单位(包括自己)由远到近顺序
        :param all_alive_units_count: dict, 当前所有存活单位数目
        :param all_alive_s: dict, 当前单位观察
        :param all_alive_a: dict, 当前单位动作
        :param all_r: int, 当前单位 reward
        :param all_alive_order_s_: dict, 下一时刻，存活单位(包括自己)由远到近顺序
        :param alive_units_count_s_: dict, 下一时刻，所有存活单位数目
        :param all_alive_s_: dict, 下一时刻，所有存活单位（包括自己）观察
        :param all_done: 全局是否结束
        :return:
        '''
        if self.is_training:
            for unit_id, local_state in all_alive_s.items():
                s = local_state
                # 观察中存活单位数目 * action_dim
                sequence_length = all_alive_units_count.get(unit_id) * 11
                # 按距离顺序展开的 s
                ...
                if not done and len(alive_order_s_) > 1: # 当前单位存活 and 还有其他单位存活
                    sequence_lengths_s = all_alive_units_count_s_[unit_id] * 11
                    s__others = [all_alive_s_[id] for id in alive_order_s_ if id != unit_id]  # 为了求其他单位在 s' 下的动作，需要存储其他单位在 s'的局部观察
                    sequence_lengths_s_ = [all_alive_units_count_s_[id] * 11 for id in alive_order_s_ if id != unit_id]
                else: # 当下单位已经死亡（下一个状态是什么没有影响）
                    sequence_lengths_s = 0
                    s__others = [np.zeros([11 * (COMA_CFG.agent_num+COMA_CFG.enemy_num)], dtype=np.float32)]  # Done = True, 状态不重要，但是不能为空，所以设置为上一个状态
                    s_ = np.zeros([11 * (COMA_CFG.agent_num+COMA_CFG.enemy_num)], dtype=np.float32) # Done = True, 状态不重要，但是不能为空，所以设置为上一个状态
                    sequence_lengths_s_ = [0]
                transition = construct_transition(s, sequence_length, a_others, a, r, s_, sequence_lengths_s, s__others, sequence_lengths_s_, done)
                # print(transition)
