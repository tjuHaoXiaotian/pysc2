...
transition_fields = ['state','sequence_length','action_others','action', 'reward', 'next_state','sequence_length_s','next_state_others','sequence_lengths_s', 'terminated']
Transition = namedtuple('Transition', transition_fields)

def construct_transition(state, sequence_length, action_others, action, reward, next_state, sequence_length_s, next_state_others, sequence_lengths_s, terminated):
  # TODO: 注意 model index 是不对齐的（维度不一致）
  transition = Transition(state=state, sequence_length= sequence_length,action_others = action_others,action=action,reward=reward,next_state=next_state,sequence_length_s = sequence_length_s,next_state_others=next_state_others,sequence_lengths_s = sequence_lengths_s,terminated=terminated,)
  return transition
...
class ReplayBuffer(object):
  ...
  def sample_batch(self, batch_size):
    ...
    state_batch, sequence_length_batch, action_others_batch, action_batch, reward_batch, next_state_batch, sequence_length_s_batch, next_state_others_batch, sequence_lengths_s_batch, terminated_batch = [], [], [], [], [], [], [], [], [], [],
    for idx in indices:
      trans = self.buffer[idx]
      state_batch.append(trans.state)
      sequence_length_batch.append(trans.sequence_length)
      ...
      sequence_length_s_batch.append(trans.sequence_length_s)
      next_state_others_batch.append(trans.next_state_others)
      sequence_lengths_s_batch.append(trans.sequence_lengths_s)
      terminated_batch.append([trans.terminated])

    return (state_batch, sequence_length_batch, action_others_batch, action_batch, reward_batch, next_state_batch, sequence_length_s_batch, next_state_others_batch, sequence_lengths_s_batch, terminated_batch)
